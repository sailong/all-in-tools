...
# =========================================================
# sing-box 面板 v24.0 (Debian/Ubuntu 专用版)
# =========================================================
...
restart_service() {
    ...
    if systemctl is-active --quiet "$SVC"; then
        ...
        [ "$SVC" = "sing-box" ] && journalctl -u sing-box --no-pager -n 10
    fi
}
...
install_sb() {
    ...
    apt-get install -y curl wget tar openssl uuid-runtime ca-certificates >/dev/null 2>&1
...
uninstall_sb() {
    ...
    if [ "$C" = "y" ]; then
        ...
        rm -f "$CF_BIN" "$CF_TOKEN_FILE" "/etc/systemd/system/cloudflared.service"
        
        systemctl stop sing-box >/dev/null 2>&1
        systemctl disable sing-box >/dev/null 2>&1
        rm -rf "$SB_BIN" "$BASE_DIR" "/etc/systemd/system/sing-box.service" "$SHORTCUT_BIN"
        ...
        vless)
            ...
            if [ -n "$FLOW" ]; then 
                ...
                if [ -n "$SNI_FROM_FILE" ]; then
                    ...
                    DOM_PREFIX="${SHOW_SNI%%.*}"
                    echo "vless://$UUID@$LINK_ADDR:$LINK_PORT?encryption=none&security=tls&sni=$SHOW_SNI&fp=firefox&type=ws&host=$SHOW_SNI&path=$ENC_PATH#vless-ws-tunnel-${DOM_PREFIX}"
                else
                    # 标准 WS
...
[Service]
ExecStart=/bin/bash -c '$CF_BIN tunnel --no-autoupdate run --token \$(cat $CF_TOKEN_FILE)'
Restart=on-failure
...
EOF
            ...
            3) systemctl stop cloudflared ;;
            4) systemctl restart cloudflared; sleep 1 ;;
            5) 
                systemctl stop cloudflared >/dev/null 2>&1
                systemctl disable cloudflared >/dev/null 2>&1
                rm -f "$CF_BIN" "$CF_TOKEN_FILE" "/etc/systemd/system/cloudflared.service"
...
while true; do
    cls
    echo -e "${BLUE}sing-box 全能面板 v24.0 (Debian/Ubuntu) $(get_status sing-box)${NC}"
    echo "1. 添加配置"
    echo "2. 查看配置"
    echo "3. 修改配置"
    echo "4. 删除配置"
